...
(function ()
{
	...
	behinstProto.onCreate = function()
	{
	    this.options = { // scaleOverlay
						 ...
						 // animation					 						
		                 animation: (this.properties[31] === 1),
		                 animationEasing : get_easing_function_name(this.properties[32]),		                 
		};

        this.animation_duration = this.properties[33];
        this.acc_duration = 0;
        this.chart = null;
	    this.labels = [];
	    this.datasets = [];
	...
	var easing_function_name = ["linear","easeInQuad","easeOutQuad","easeInOutQuad",
	...
	var get_easing_function_name = function(s)
	{
	    return easing_function_name[s];
	};	
	...
	behinstProto.tick = function ()
	{
	    ...
	    this.acc_duration += dt;
	    var p = (this.animation_duration > 0)? this.acc_duration/this.animation_duration : 1;
	    this.is_drawing = this.chart.Draw(p);
	    ...
	    if (!this.is_drawing) 
	    {
	        ...
	        this.runtime.trigger(cr.behaviors.Rex_canvas_chart_radar.prototype.cnds.OnDrawingFinished, this.inst); 
	        this.is_my_call = false;
	    }
	};
	behinstProto.get_data = function (dataset_name, label_name)
	{
	...
	behinstProto.draw_chart = function(data)
	{
	    ...
	    this.chart = this.inst.extra.rex_chartjs.Radar(data, this.options);
	...
	behinstProto.saveToJSON = function ()
	{
	    var options = [// scaleOverlay
					   ...
					   this.options.angleLineColor,
					   this.options.angleLineWidth,
					   // point label
					   this.options.pointLabelFontFamily,
					   this.options.pointLabelFontSize,
					   this.options.pointLabelFontStyle,
					   this.options.pointLabelFontColor,
					   // point dot
	...
	behinstProto.loadFromJSON = function (o)
	{
	    ...
	    // angle line
	    this.options.angleShowLineOut= options[18];
	    this.options.angleLineColor= options[19];
	    this.options.angleLineWidth= options[20];
	    // point label
	    this.options.pointLabelFontFamily= options[21];
	    this.options.pointLabelFontSize= options[22];
	    this.options.pointLabelFontStyle= options[23];
	    this.options.pointLabelFontColor= options[24];
	    // point dot
	    this.options.pointDot= options[25];
	    this.options.pointDotRadius= options[26];
	    this.options.pointDotStrokeWidth= options[27];
	    // dataset
	    this.options.datasetStroke= options[28];
	    this.options.datasetStrokeWidth= options[29];
	    this.options.datasetFill= options[30];
		// animation
		this.options.animation= options[31];
		this.options.animationEasing= options[32];	  
        this.animation_duration= options[33];
	};	
	//////////////////////////////////////
	// Conditions
	function Cnds() {};
	...
	Cnds.prototype.OnDrawingFinished = function ()
	{        
	...
	Acts.prototype.SetEnabledAngleLine = function(s)
	{
		this.options.angleShowLineOut = (s===1);
	}; 	
	Acts.prototype.SetAngleLine = function(color, width)
	{
	    this.options.angleLineColor= color;
	    this.options.angleLineWidth= width;
	};
	Acts.prototype.SetPointLabelFont = function(font_family, font_size, font_style, font_color)
	{
	    this.options.pointLabelFontFamily= font_family;
	    this.options.pointLabelFontSize= font_size;
	    this.options.pointLabelFontStyle= font_style;
	    this.options.pointLabelFontColor= font_color;
	};	
	...
	{
		this.options.animation= (s===1);
	}; 	
	Acts.prototype.SetAnimationDuration = function(t)
	{
...
(function ()
{
    if (window.hasOwnProperty("chartjs"))
        return;
...
window["chartjs"] = function(context){
	...
	var PolarArea = function(data,config,ctx){
		...
		this.Draw = animationLoop(config,drawScale,drawAllSegments,ctx);
	...
	var Radar = function (data,config,ctx) {
		...
		this.Draw = animationLoop(config,drawScale,drawAllDataPoints,ctx);
	...
	var Pie = function(data,config,ctx){
		...
		this.Draw = animationLoop(config,null,drawPieSegments,ctx);
	...
	var Line = function(data,config,ctx){
		...
		calculateXAxisSize();
		this.Draw = animationLoop(config,drawScale,drawLines,ctx);		
	...
	var Bar = function(data,config,ctx){
		...
		scaleHop = Math.floor(scaleHeight/calculatedScale.steps);
		calculateXAxisSize();
		this.Draw = animationLoop(config,drawScale,drawBars,ctx);		
	...
	function animationLoop(config,drawScale,drawData,ctx){
		...
		function tweenDraw(_percentAnimComplete)
		{
		    if (!config.animation)
		    {
		        animateFrame();
		    ...
		    percentAnimComplete = _percentAnimComplete;
		    animateFrame();
		    var is_continue = (percentAnimComplete < 1);
		    return is_continue;
		}
