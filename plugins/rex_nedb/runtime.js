...
(function ()
{
	...
	{
	    if (cr.plugins_.Rex_nedb.databases.hasOwnProperty(db_name))
	        return;

	    var db_ref = {db:window["TAFFY"](db_content), 
	                  ownerID:ownerUID};	            
	    cr.plugins_.Rex_nedb.databases[db_name] = db_ref;
	};
	
	var get_global_database_reference = function (db_name)
	{
	    return cr.plugins_.Rex_nedb.databases[db_name];
	};
	...
	{   
	    var invalid_rowID = (rowID == null) || 
	                        (rowID === "") || 
	                        !this.isRowIDValid(rowID);
    ...
    instanceProto.isRowIDValid = function (rowID)
	{    
	    debugger
        var row = this.db(rowID)["get"]()[0];
        return (row != null) && row["___id"] === rowID;
	};	
	...
	instanceProto.loadFromJSON = function (o)
	{
		...
		else
		{
		    ...
		    {
		        if (cr.plugins_.Rex_nedb.databases.hasOwnProperty(db_name))		        
		            delete cr.plugins_.Rex_nedb.databases[db_name];		        
